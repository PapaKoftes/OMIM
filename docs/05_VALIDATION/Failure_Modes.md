...
Category C: Validation Failures     (rule engine failures, not rule violations)
...
Category F: System Failures         (config/dependency issues)
...
- **Behavior**: Return `ParseResult(success=False, error_code="DXF_CORRUPT", error_message=str(e))`
...
- **Behavior**: Return `ParseResult(success=False, error_code="DXF_VERSION_UNSUPPORTED")`
- **Recovery**: Convert file using LibreCAD (external); retry

### A-004: DXF File Too Large
- **Trigger**: File size > 50MB
- **Behavior**: Return `ParseResult(success=False, error_code="DXF_TOO_LARGE")`
...
- **Dataset impact**: Sample is marked `is_valid=False` with violation `"GEO-001"`
...
- **Confidence impact**: All edge-clearance rules fall back to bounding-box inference; lower confidence on those results
- **`panel_boundary_inferred = true`** recorded in MGG metadata

### B-007: Coordinate System Mismatch (Inches)
...
These are rule engine failures — not manufacturing violations (which are expected outputs).
...
- **Trigger**: Rule YAML has `rule_id: MFG-999` but no handler implementation
- **Behavior**: `RuleEngine` raises `UnknownRuleError` at load time
- **Recovery**: Remove rule from YAML or implement the handler; no silent skip
...
**Note**: Manufacturing rule violations (GEO-001, MFG-001, etc.) are NOT failures — they are expected structured outputs (`RuleResult` with `severity="ERROR"`). `SYSTEM_ERROR` results do not cause `overall_valid=False` unless explicitly required by operator policy.
...
- **Behavior**: Log `"ML unavailable; using heuristics only"`; fall back to heuristic classifiers
...
### D-006: Semantic Precondition Not Met
- **Trigger**: Semantic layer called when `layer1_passed=False`
- **Behavior**: Raise `SemanticPreconditionError("Layer 1 validation must pass before semantic inference")`
- **Recovery**: Caller must check `validation_report.layer1_passed` before calling semantic layer
...
- **Trigger**: Feature sampler cannot find a valid position after 100 placement attempts
...
### E-005: Generated Valid Panel Fails Own Validation
...
- **Recovery**: Investigate generation logic; likely a floating-point tolerance issue in feature placement

### E-006: Reproducibility Check Failure
- **Trigger**: Regenerating from same seed produces a different DXF
- **Behavior**: CRITICAL — treat as a bug; do NOT release this dataset version
- **Recovery**: Audit all RNG usage for non-seeded calls; check for OS-dependent floating-point behavior
...
- **Trigger**: Dataset sample directory missing one or more of the five required files
...
class PipelineError(BaseModel):
    error_code: str         # e.g., "DXF_CORRUPT", "A-002"
...
class PipelineWarning(BaseModel):
    warning_code: str       # e.g., "no_cuttable_entities", "A-006"
    message: str
...
| Failure | During Generation | During Inference | During Benchmarking |
|---------|-------------------|-----------------|-------------------|
| A-001 to A-004 | Skip file, log, continue | Return error to caller | Fail loudly |
| A-005, A-006 | Continue (empty MGG) | Return empty result | Include as empty-panel cases |
| B-001 to B-005 | Label as invalid, continue | Flag in output | Include as negative examples |
| B-006 | Infer boundary, warn | Infer boundary, warn | Acceptable with note |
...
| C-004 | Log, report SYSTEM_ERROR | Log, report SYSTEM_ERROR | Acceptable; flag as timeout |
...
| E-001 to E-003 | Log, continue generation | N/A | N/A |
| E-005, E-006 | STOP — generation bug | N/A | N/A |
| F-001 to F-004 | STOP — config/code bug | STOP | STOP |
| G-001 to G-004 | STOP — code bug | STOP | STOP |
...
try:
    ...
    return None  # NEVER return None without logging
